...
function miniHTML(board) {
  ...
  for (let r = 0; r < 8; r++) for (let c = 0; c < 8; c++) {
    const p = board[r][c]; const cell = (r + c) % 2 === 0 ? 'ml' : 'md';
    h += `<i class="${cell}">${p ? PCIMG(p) : ''}</i>`;
  }
  return h;
}

// ── Server-side share ──
async function saveShare(moves, meta, analysis) {
  try {
    const res = await fetch('/api/share', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ moves, meta, analysis })
    });
  ...
  games.forEach((g, idx) => {
    ...
    card.innerHTML = `
      ...
      <div class="gc-meta clickable-area">
        <div class="player-row">⚪ ${g.white || 'White'}</div>
        <div class="player-row">⚫ ${g.black || 'Black'}</div>
        <div class="game-date">${d.toLocaleDateString()}</div>
      </div>
      <div class="gc-actions">
        <button class="btn-action-del" title="Delete" data-idx="${idx}">🗑</button>
      </div>`;
    const clickHandler = () => {
    ...
    card.querySelector('.btn-action-del').onclick = e => {
      ...
      if (confirm('Remove this game from history?')) {
        const all = loadGames(); all.splice(idx, 1);
        localStorage.setItem(LSK, JSON.stringify(all)); renderMine();
      }
    };
...
// ── Chess.com import ──
(function fillMonths() {
  const sel = $('cc-month'), now = new Date();
  const months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
  for (let i = 0; i < 24; i++) {
    const d = new Date(now.getFullYear(), now.getMonth() - i, 1);
    const y = d.getFullYear(), m = String(d.getMonth() + 1).padStart(2, '0');
    const o = document.createElement('option'); o.value = y + '/' + m;
    o.textContent = months[d.getMonth()] + ' ' + y; sel.appendChild(o);
  }
})();
...
$('cc-load').addEventListener('click', async () => {
  ...
  $('cc-msg').textContent = 'Loading…'; $('cc-list').innerHTML = ''; btn.disabled = true;
  try {
    const res = await fetch(`/api/chesscom/${encodeURIComponent(user)}/${y}/${m}`);
    if (!res.ok) { $('cc-msg').textContent = res.status === 404 ? 'No games found.' : 'Error ' + res.status; return; }
    ...
    games.forEach(g => {
      ...
      const mine = (w.username || '').toLowerCase() === userLc ? w : (b.username || '').toLowerCase() === userLc ? b : null;
      ...
      } else { res = w.result === 'win' ? '1–0' : b.result === 'win' ? '0–1' : '½–½'; }
...
function loadFromPgn(pgn, meta) {
  ...
  try { ok = c.load_pgn(pgn, { sloppy: true }); } catch { ok = false; }
  if (!ok) { alert('Could not parse this game.'); return; }
  const verbose = c.history({ verbose: true });
  const c2 = new Chess(); const fens = [c2.fen()], sans = [], ucis = [];
  for (const m of verbose) {
    const mv = c2.move({ from: m.from, to: m.to, promotion: m.promotion });
    ucis.push(m.from + m.to + (mv.promotion || '')); sans.push(mv.san); fens.push(c2.fen());
  }
...
function enterAnalyzer() {
  ...
  renderMoveList(); renderBoard(); drawEvalBar(null); setupExtraButtons();
}
$('az-toGames').addEventListener('click', () => {
  if (engine) engine.stop();
  $('analyzer').style.display = 'none'; $('picker').style.display = ''; renderMine();
});
...
function renderBoard() {
  ...
  for (let r = 0; r < 8; r++) for (let c = 0; c < 8; c++) {
    const ar = flipBoardOrientation ? 7 - r : r;
    const ac = flipBoardOrientation ? 7 - c : c;
    const fileChar = String.fromCharCode(97 + ac); const rankNum = 8 - ar;
    const sqName = fileChar + rankNum; const p = board[ar][ac];
    const d = document.createElement('div');
    d.className = 'sq ' + ((ar + ac) % 2 === 0 ? 'l' : 'd');
    if (lm && (sqName === lm.from || sqName === lm.to)) d.className += ' hl';
    if (p) d.innerHTML = PCIMG(p);
    if (c === 0) { const lb = document.createElement('span'); lb.className = 'coord-rank'; lb.textContent = rankNum; d.appendChild(lb); }
    if (r === 7) { const lb = document.createElement('span'); lb.className = 'coord-file'; lb.textContent = fileChar; d.appendChild(lb); }
    if (lm && sqName === lm.to && analysis && analysis.classes[ply - 1]) {
      const cl = analysis.classes[ply - 1]; d.innerHTML += `<i class="badge b-${cl}">${ICON[cl] || ''}</i>`;
    }
    el.appendChild(d);
  }
}
...
function showMoveInfo() {
  ...
  const names = { brilliant: 'Brilliant!!', great: 'Great move!', best: 'Best move', excellent: 'Excellent', good: 'Good', book: 'Book move', inaccuracy: 'Inaccuracy', mistake: 'Mistake', blunder: 'Blunder' };
  const movedSan = CURRENT.sans[ply - 1];
  const bestSan = analysis.bestSans ? analysis.bestSans[ply - 1] : null;
  ...
  if (!isBest && (cl === 'inaccuracy' || cl === 'mistake' || cl === 'blunder'))
    ...
    suggestion = ` · <span style="color:var(--muted)">Engine's top choice</span>`;
  ...
  $('moveinfo').innerHTML = `<span class="c-${cl}">${movedSan} — ${names[cl] || cl}</span>${lossNote}${suggestion}`;
}

// Nav buttons
function navAction(btn, fn) {
  btn.addEventListener('click', e => {
    fn(); e.currentTarget.blur();
    if (window.innerWidth <= 760) { const b = $('board'); if (b) b.scrollIntoView({ block: 'nearest', behavior: 'instant' }); }
  });
}
...
document.addEventListener('keydown', e => {
  if ($('analyzer').style.display === 'none') return;
  if (e.target.tagName === 'INPUT' || e.target.tagName === 'SELECT') return;
  if (e.key === 'ArrowLeft') $('nPrev').click();
  if (e.key === 'ArrowRight') $('nNext').click();
});
...
function drawGraph(whiteCp) {
  ...
  svg.innerHTML += `<rect x="0" y="0" width="${W}" height="${mid}" fill="rgba(255,255,255,0.05)"/>
    <line x1="0" y1="${mid}" x2="${W}" y2="${mid}" stroke="rgba(201,168,76,.4)" stroke-width="0.5"/>`;
  const pts = whiteCp.map((cp, i) => `${x(i).toFixed(1)},${y(cp).toFixed(1)}`).join(' ');
  svg.innerHTML += `<polygon points="0,${mid} ${pts} ${W},${mid}" fill="rgba(201,168,76,0.22)"/>
    <polyline points="${pts}" fill="none" stroke="#c9a84c" stroke-width="1.2"/>`;
  // current ply marker
  if (ply > 0 && ply <= whiteCp.length - 1) {
    const cx = x(ply).toFixed(1), cy = y(whiteCp[ply]).toFixed(1);
    svg.innerHTML += `<circle cx="${cx}" cy="${cy}" r="3" fill="#c9a84c" stroke="#fff" stroke-width="1"/>`;
  }
}
$('graph').addEventListener('click', e => {
  ...
  ply = Math.max(0, Math.min(CURRENT.sans.length, Math.round(ratio * CURRENT.sans.length)));
  refresh(); drawGraph(analysis.whiteCp);
});

// ── Summary ──
function renderSummary(classes) {
  const order = ['brilliant', 'great', 'best', 'excellent', 'good', 'book', 'inaccuracy', 'mistake', 'blunder'];
  const names = { brilliant: '!!', great: '!', best: 'Best', excellent: 'Excl', good: 'Good', book: 'Book', inaccuracy: 'Inacc', mistake: 'Mistake', blunder: 'Blunder' };
...
// ── Extra buttons (share + flip) ──
function setupExtraButtons() {
  const container = document.querySelector('.az-controls');
  if (!container || $('btn-row-2')) return;
  const row2 = document.createElement('div');
  row2.id = 'btn-row-2'; row2.className = 'az-control-row-group';

  const shareBtn = document.createElement('button');
  shareBtn.id = 'btn-share-link'; shareBtn.className = 'btn-secondary flex-btn';
  shareBtn.textContent = '🔗 Share';
  shareBtn.onclick = async () => {
    shareBtn.disabled = true; shareBtn.textContent = 'Saving…';
    const url = await saveShare(CURRENT.ucis, CURRENT.meta, analysis);
    if (url) {
    ...
    setTimeout(() => { shareBtn.textContent = '🔗 Share'; shareBtn.disabled = false; }, 2500);
  };

  const flipBtn = document.createElement('button');
  flipBtn.id = 'btn-flip-board'; flipBtn.className = 'btn-secondary flex-btn';
  flipBtn.innerHTML = '🔄 Flip';
  flipBtn.onclick = () => { flipBoardOrientation = !flipBoardOrientation; renderBoard(); };

  row2.appendChild(shareBtn); row2.appendChild(flipBtn);
...
class Engine {
  constructor() {
    this.w = new Worker('/engine/stockfish16.js');
    this.cur = null; this.q = []; this.info = []; this._readyRes = null;
  ...
  _msg(line) {
    ...
    if (!this.cur) return;
    if (line.startsWith('info') && line.includes(' score ') && line.includes(' pv ')) this.info.push(line);
    else if (line.startsWith('bestmove')) {
      const bm = line.split(' ')[1]; const r = parseInfo(this.info, bm);
      const d = this.cur; this.cur = null; this.info = []; d.resolve(r); this._next();
    }
  }
  evaluate(fen, depth) {
    return new Promise(res => {
      const job = { fen, depth, done: false };
      job.timer = setTimeout(() => {
        if (!job.done) { job.done = true; res({ bestmove: null, best: null, eval: { cp: 0, mate: null }, eval2: null }); this.cur = null; this._next(); }
      }, 10000);
      job.resolve = r => { if (job.done) return; job.done = true; clearTimeout(job.timer); res(r); };
  ...
  stop() { try { this.send('stop'); this.w.terminate(); } catch { } engine = null; }
}

function parseInfo(infos, bestmove) {
  const byPv = {};
  for (const l of infos) { const m = / multipv (\d+)/.exec(l); byPv[m ? +m[1] : 1] = l; }
  const ev = l => {
    if (!l) return null;
    const mt = / score mate (-?\d+)/.exec(l); if (mt) { const m = +mt[1]; return { mate: m, cp: m > 0 ? 100000 - m * 100 : -100000 - m * 100 }; }
    const cp = / score cp (-?\d+)/.exec(l); return { cp: cp ? +cp[1] : 0, mate: null };
  };
...
function hangsSacrifice(fenAfter) {
  try {
    ...
    for (const m of caps) {
      if (VAL[m.captured] >= 3) {
        const c2 = new Chess(fenAfter); c2.move({ from: m.from, to: m.to, promotion: 'q' });
        const re = c2.moves({ verbose: true }).filter(x => x.to === m.to && x.captured);
        const minRe = re.length ? Math.min(...re.map(x => VAL[x.captured])) : Infinity;
        if (!re.length || minRe < VAL[m.captured]) return true;
      }
    }
  } catch { }
  return false;
...
async function runAnalysis() {
  if (engine) { engine.stop(); engine = null; await new Promise(r => setTimeout(r, 100)); }
  const depth = +$('depth').value || 18;
  const QUICK = Math.max(10, depth - 6);
  const BOOK_PLIES = 8;
  const fens = CURRENT.fens, ucis = CURRENT.ucis;
  engine = new Engine(); await engine.readyP;
  $('progress').classList.remove('hidden'); $('run').disabled = true;

  const evals = [], bests = [], eval2s = [], bestSans = [];

  // ── Pass 1: quick scan all positions ──
  const quickEvals = [];
  for (let i = 0; i < fens.length; i++) {
    const probe = new Chess(fens[i]);
    if (probe.game_over()) {
      quickEvals.push({ cp: probe.in_checkmate() ? (probe.turn() === 'w' ? -100000 : 100000) : 0, mate: null });
    } else {
      const r = await engine.evaluate(fens[i], i < BOOK_PLIES ? 6 : QUICK);
      quickEvals.push(r.eval);
    }
    $('progbar').style.width = Math.round((i + 1) / fens.length * 40) + '%';
    $('progtxt').textContent = `Quick scan ${i + 1}/${fens.length}`;
  }

  // ── Identify critical positions (5%+ win-chance swing) ──
  const critical = new Set();
  for (let i = 1; i < fens.length; i++) {
    const prev = quickEvals[i - 1].cp || 0;
    const curr = quickEvals[i].cp || 0;
    const swing = Math.abs(winPct(prev) - winPct(-curr));
    if (i >= BOOK_PLIES && swing >= 5) critical.add(i);
  }

  // ── Pass 2: deep eval on critical positions only ──
  for (let i = 0; i < fens.length; i++) {
    const probe = new Chess(fens[i]);
    if (probe.game_over()) {
      evals.push(quickEvals[i]); bests.push(null); eval2s.push(null); bestSans.push(null);
    } else if (critical.has(i)) {
      const r = await engine.evaluate(fens[i], depth);
      evals.push(r.eval); bests.push(r.best); eval2s.push(r.eval2); bestSans.push(uciToSan(fens[i], r.best));
    } else {
      const r = await engine.evaluate(fens[i], QUICK);
      evals.push(quickEvals[i]); bests.push(r.best); eval2s.push(r.eval2); bestSans.push(uciToSan(fens[i], r.best));
    }
    $('progbar').style.width = Math.round(40 + (i + 1) / fens.length * 60) + '%';
    $('progtxt').textContent = critical.has(i)
      ? `Deep analysis ${i + 1}/${fens.length} ⚡`
      : `Scanning ${i + 1}/${fens.length}`;
  }
  ...
  for (let i = 0; i < fens.length; i++) {
    const cp = evals[i].cp; whiteCp.push(sideAt(fens[i]) === 'w' ? cp : -cp);
  }
  const bookPlies = Math.min(BOOK_PLIES, ucis.length);
  ...
  // ── Weighted accuracy ──
  const weightedAcc = losses => {
    if (!losses.length) return 100;
    const n = losses.length;
    const [wSum, wTotal] = losses.reduce(([ws, wt], l, i) => {
      const w = 1 + (n - i) / n;
      return [ws + accFromLoss(l) * w, wt + w];
    }, [0, 0]);
    return Math.round(wSum / wTotal * 10) / 10;
  };

  const accW = weightedAcc(lossW);
  const accB = weightedAcc(lossB);
...
(async function init() {
  ...
  if (shareId) {
    ...
    if (data) {
      flipBoardOrientation = false;
      loadFromUci(data.moves, data.meta);
      if (data.analysis) {
        analysis = data.analysis;
        const sideAt = f => f.split(' ')[1];
        const whiteCp = [];
        for (let i = 0; i < CURRENT.fens.length; i++) {
          const cp = analysis.evals[i].cp;
          whiteCp.push(sideAt(CURRENT.fens[i]) === 'w' ? cp : -cp);
        }
        analysis.whiteCp = whiteCp;
        $('acc-white').textContent = analysis.accW + '%';
        $('acc-black').textContent = analysis.accB + '%';
        renderSummary(analysis.classes); drawGraph(whiteCp); renderMoveList(); refresh();
      }
    } else {
      alert('Share link not found or expired (links last 30 days).');
    }
  } else if (q.get('source') === 'last') {
    const g = loadGames()[0];
    if (g) { flipBoardOrientation = (g.myColor === 'b'); loadFromUci(g.moves, { white: g.white, black: g.black, result: g.result }); }
  } else if (q.get('gid')) {
    const g = loadGames().find(x => x.id === q.get('gid'));
    if (g) { flipBoardOrientation = (g.myColor === 'b'); loadFromUci(g.moves, { white: g.white, black: g.black, result: g.result }); }
  }
})();
